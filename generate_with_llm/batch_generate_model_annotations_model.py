...
def reformat_model_id(model_id):
    return osp.split(model_id)[-1].replace('.', '').replace('_', '')
...
async def main(args):
    ...
    # read input file with model descriptions

    with open(args.model_description_file, "r") as f:
        sample_file_content = json.load(f)
        description_model_id = sample_file_content["model_id"]
        description_model_thinking = sample_file_content["args"]["enable_thinking"]
        model_descriptions = sample_file_content["responses"]
    ...
    async def single_request(
        ...
        no_valid_description = description is None or description.strip() == ""
        invalid_item = df[df.index == target_id].empty
        
        if no_valid_description or invalid_item:
            ...
            out_obj = {
                ...
                "model_response": None,
                "reasoning": None,
                "error_type": 'invalid description' if no_valid_description else 'invalid target_id',
                "error_msg": None,
                "status_code": None,
            }
    ...
    async def batch_requests(data, max_concurrent=args.max_concurrent):
        ...
        tasks = [
            single_request(d["target_id"], d["description"], semaphore)
            for d in data
        ]
        return await tqdm_asyncio.gather(*tasks)

    # generate and collect responses
    all_responses = await batch_requests(model_descriptions)
    ...
    eval_system_str = f"_{reformat_model_id(description_model_id)}"
    eval_thinking_str = "-thinking" if description_model_thinking else ""

    out_path = osp.join(
        args.out_dir,
        f"{reformat_model_id(model_id)}{thinking_str}{eval_system_str}{eval_thinking_str}_annotations.json",
        )

    out_obj = {"model_id": model_id, "evaluated_system": description_model_id, "evaluated_enable_thinking": description_model_thinking, "args": {**vars(args)}, "responses": all_responses}
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Path to the file with model descriptions.",
    )
